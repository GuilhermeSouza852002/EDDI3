...
void capturaDados(Funcionario *ptFuncionario){
...
void mostra(Funcionario *ptFuncionario)
{
	Funcionario *ptAux = ptFuncionario;
	
	for(int i = 0; i < (Tam);  i++)
	{
			cout << "\tNome: " << ptAux->nome << "\tSalario: " << ptAux->salario << endl;
			ptAux = ptAux+1;
	}
}
...
int main(){
	...
	Funcionario ptFuncionario [Tam];
	capturaDados(ptFuncionario);
	mostra(ptFuncionario);
